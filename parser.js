...
export function parseLine(line) {
  // line — строка из файла input.txt
  const parts = getParts(line); // удаляем пробелы по краям и разбиваем строку по пробелам в массив parts
  const shapeType = parts[0]; // первый элемент (parts[0]) — это тип фигуры: 'Square', 'Rectangle', 'Circle', 'Triangle'

  switch (
    shapeType // разбор по типу фигуры:
  ) {
    case "Square":
      return parserSquare(line);

    case "Rectangle":
      return parserRectangle(line);

    case "Circle":
      return parserCircle(line);

    case "Triangle":
      return parserTriangle(line);
...
export function parserRectangle(line) {
  const parts = getParts(line);
...
export function parserTriangle(line) {
  const parts = getParts(line);
